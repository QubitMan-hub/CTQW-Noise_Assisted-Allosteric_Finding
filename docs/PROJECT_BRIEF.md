...
Development happens on Windows, run through PowerShell. Use Windows-appropriate paths.

## Repository structure to create

```
ctqw-allosteric/
  README.md              short project summary (can start from this brief)
  src/                   source code
    rin_builder.py
  data/
    structures/          input PDB or mmCIF files
    benchmark/           ASD allosteric labels (added later)
  outputs/               generated networks (graphml, npz, csv, png)
    example_1A8O/         the worked example, if uploaded
  notebooks/             analysis notebooks (added later)
  docs/                  notes and paper drafts
...
2. Place the files I have uploaded into the correct folders:
   - `rin_builder.py` goes in `src/`.
   - any `.pdb` or `.cif` structure files go in `data/structures/`.
   - the example output files (graphml, npz, csv, png, summary) go in
     `outputs/example_1A8O/`.
   - this brief goes in `docs/`.
3. If a file's correct location is unclear, ask me rather than guessing.

Do not run the code, do not install anything, do not edit or extend any file, and do
not start on any pipeline stage. I will give you the next steps one at a time.
